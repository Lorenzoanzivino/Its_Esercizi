...
class Noleggio:
    __film_list:list[Film]    # lista di film del negozio
    ...
    def giveBack(self, film: 'Azione | Commedia | Drama', clientID: int, giorniRitardo: int) -> None:
        if clientID in self.__rented_film and film in self.__rented_film[clientID]:
            ...
            penale_totale = film.calcolaPenaleRitardo(giorniRitardo)
            print(f"Cliente: {clientID}! La penale da pagare per il film {film.getTitle()} è di {penale_totale} euro!")
        else:
    ...
    def printRentMovies(self, clientID: int) -> None:
        if clientID in self.__rented_film and self.__rented_film[clientID]:
