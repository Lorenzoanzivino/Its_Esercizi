# =========================================================
# ESEMPIO COMPLETO DI CIFRATURA E DECIFRATURA RSA
# Tutte le funzioni base incluse nel file
# =========================================================
...
# 1) Converte modulo e esponente privato in esadecimale
def rsa_params_to_hex(modulus: int, private_exponent: int) -> tuple[str, str]:
    n_hex = hex(modulus)[2:]  # rimuove il prefisso '0x'
    d_hex = hex(private_exponent)[2:]
    return n_hex, d_hex

# 2) Converte una stringa in un intero
def string_to_int(message: str) -> int:
    return int.from_bytes(message.encode('utf-8'), byteorder='big')


# 3) Cifra un messaggio intero con RSA (modulo ed esponente pubblico/privato)
def rsa_encrypt(message_int: int, modulus: int, exponent: int) -> int:
    # la cifratura RSA è semplicemente: c = m^e mod n
    return pow(message_int, exponent, modulus)


# 4) Converte una stringa esadecimale in intero
def hex_to_int(hex_str: str) -> int:
    try:
        # rimuove spazi, newlines e ":"
        clean_hex = ''.join(c for c in hex_str if c in '0123456789abcdefABCDEF')
        return int(clean_hex, 16)
    except ValueError:
        print(f"Errore: '{hex_str}' non è una stringa esadecimale valida.")
        return None
...
# 5) Converte un intero in stringa
def int_to_string(message_int: int) -> str:
    try:
        return message_int.to_bytes((message_int.bit_length() + 7) // 8, byteorder='big').decode('utf-8')
    except Exception as e:
        print(f"Errore nella conversione da intero a stringa: {e}")
        return None


# 6) Decifra un messaggio intero con RSA
def rsa_decrypt(cipher_int: int, modulus: int, exponent: int) -> int:
    return pow(cipher_int, exponent, modulus)


# =========================================================
# 2) PARAMETRI DELLA CHIAVE RSA
# =========================================================
...
# =========================================================
# 4) MESSAGGIO DA CIFRARE
# =========================================================
msg = "Ciao Lorenzo!"
print("\nMessaggio originale:", msg)

# Converti stringa -> intero
M = string_to_int(msg)
...
# Converti intero -> stringa
msg_decifrato = int_to_string(M2)
print("Messaggio decifrato:", msg_decifrato)